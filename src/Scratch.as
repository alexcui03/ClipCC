...
public class Scratch extends Sprite {
	// Version
	public static const versionString:String = '1.4.2.1';
	...
	public var codePart:CodesPart;
	public var classPart:ClassPart;
	...
	public function refreshCodeTab():void {
		codePart.refresh();
	}
	
	public function refreshClassTab():void {
		classPart.refresh();
	}
...
//		}
	...
	public function selectSprite(obj:ScratchObj):void {
		...
		if (isShowing(codePart)) {
			codePart.currentIndex = 0;
			codePart.refresh();
		}
		if (isShowing(classPart)) {
			classPart.currentIndex = 0;
			classPart.refresh();
		}
	...
	protected function addParts():void {
		...
		codePart = new CodesPart(this);
		classPart = new ClassPart(this);
		addChild(topBarPart);
		addChild(stagePart);
	...
	protected function updateContentArea(contentX:int, contentY:int, contentW:int, contentH:int, fullH:int):void {
		imagesPart.x = soundsPart.x = scriptsPart.x = codePart.x = classPart.x = contentX;
		imagesPart.y = soundsPart.y = scriptsPart.y = codePart.y = classPart.y = contentY;
		imagesPart.setWidthHeight(contentW, contentH);
		soundsPart.setWidthHeight(contentW, contentH);
		codePart.setWidthHeight(contentW, contentH);
		scriptsPart.setWidthHeight(contentW, contentH);
		classPart.setWidthHeight(contentW, contentH);
	...
	public function translationChanged():void {
		...
		codePart.updateTranslation();
		classPart.updateTranslation();
	}
	...
	public function showFileMenu(b:*):void {
		...
		//m.addItem('New ScratchX Project(Experimental)', createNewProjectScratchX);
		m.addLine();
...
//		m.addItem('Save', save);
	...
	public function showmyMenu(b:*):void {
		var m:Menu = new Menu(null, 'More1', CSS.topBarColor(), 28);
		m.addItem('一键上传到百度贴吧', tieba);
		m.addItem('访问ScWiki中国版', wiki);
		m.addItem('反馈bug', qqqun);
		m.addItem('关于公众号', openGzhPng);
	//	m.addItem('退出', quit);
		//m.addItem('摸摸司南', openGzhPng);
		
		if (b.lastEvent.shiftKey) {
			m.addLine();
			m.addItem('sb2转EXE', sb2toexe);
			m.addItem('sb2转swf', showUpdatesDialog);	
		}
	...
	public function showEditMenu(b:*):void {
		...
		m.addItem('Updates', showUpdatesDialog);
		m.addItem('访问官网', jumpclip);
//		m.addItem('LoadJS Extensions', makeLoadExperimentalExtensionButton);
	...
	private function jumpclip():void {
		navigateToURL (new URLRequest(" http://ge.doforyou.top/index.html"), "_blank");
	}

	private function tieba():void {
		navigateToURL (new URLRequest(" http://tieba.baidu.com/f?kw=scratch&ie=utf-8&pn=50"), "_blank");
		var d:DialogBox = new DialogBox(/*uiwidgets.ok, uiwidgets.cancel*/);
		d.addText("此功能是由过气的贴吧WEP端实现的");
		d.addText("可能会出现双发，三发等奇怪的现象");
		d.addText("碰到验证码会漏发");
		d.addText("最后，此功能为实验性，正在完善中，");
		d.addText("敬请期待");
		d.addTitle("上传你的作品到百度贴吧");
		d.addField("作品名字",120);
		d.addField("百度贴吧ID",120);
		d.addField("密码",120);
		d.addButton('上传', d.cancel);
		d.addButton('关闭', d.cancel);
		d.showOnStage(stage, true);
	}
	private function sb2toexe():void {
		navigateToURL (new URLRequest("\tools\toexe\toexe.exe"), "_blank");
	}

	private function wiki():void {
		navigateToURL (new URLRequest("http://wiki.bianchengla.cn/"), "_blank");
	}

	private function qqqun():void {
		navigateToURL (new URLRequest("https://jq.qq.com/?_wv=1027&k=563h4tP"), "_blank");
		DialogBox.notify(
			'bug反馈 ',
			'\n\n ClipCC相比GE可谓是相对稳定' +
			'\n，如果仍然有bug请作者联系QQ2302998733' +
			'\n\n此按钮只提供加入反馈群', stage);
	}

	private function openGzhPng():void {
		var d:DialogBox = new DialogBox();
		d.addTitle('公众号二维码—欢迎关注');
		d.addWidget(new IconButton(app.logoButtonPressed,Resources.createBmp("wechat")));
		d.addButton('Close', d.cancel);
		d.addText('公众号运行者QQ：1183843590');
		d.showOnStage(stage, true); 
	}

	
	private function showUpdatesDialog():void {
		DialogBox.notify(
			'1.4更新内容 ',
			'\n\n 1.修复小米兰亭字体不完整的问题' +
			'\n2.增加录制时间为300秒' +
	...
	protected function createNewProjectScratchX(jsCallback:Array):void {
		createNewProjectAndThen(function():void {
			...
			startNewProject('', '');
			setProjectName('New ScratchX Project');
			topBarPart.refresh();
			stagePart.refresh();;
		});
	}
...
//		}
	...
	// -----------------------------
	// ClipCC专用函数 (部分处于不稳定或者测试，请勿随意使用)
	//------------------------------

	
	public function closetip(postSaveAction:Function = null):void {
		...
		var d:DialogBox = new DialogBox();
		d.addTitle('您确定要关闭吗？');	
		d.addButton('是的', proceedWithoutSaving);
		d.addButton('返回', cancel);
		d.showOnStage(stage);
	}
	public function stopAndSaveAndThen(param1:Function = null) : void
	{
		var postSaveAction:Function = param1;
		extensionManager.stopButtonPressed();
		setTimeout(function():void
		{
			if(saveNeeded)
			{
				saveProjectAndThen(postSaveAction);
			}
			else if(postSaveAction != null)
			{
				postSaveAction();
			}
		},50);
