...
package ui.parts {
	...
	public class ScriptsPart extends UIPart {
		...
		private function redraw():void {
			...
			g.drawRect(0, 0, w, h);//w h
			g.endFill();
			g.beginFill(0xe1e1e1);//新加的
			g.drawRect(0, 0, 209, 1);//新加的
			g.endFill();//新加的
			g.beginFill(CSS.white);//选择脚本//新加的
			g.drawRect(0, 1, 209, 500);//新加的
			g.endFill();
			var lineY:int = selector.y + selector.height;
			var darkerBorder:int = CSS.borderColor;
			var lighterBorder:int = CSS.borderColor;//0xF2F2F2;
