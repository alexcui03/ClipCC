...
package ui {
	...
	public class PaletteSelectorItem extends Sprite {
		...
		public function setSelected(flag:Boolean):void {
			...
			if (isSelected)
			{
				g.drawRect(tabInset+tabW/2, 3, w - tabInset - tabW-3, tabW*2+2);
				g.endFill();
				g.beginFill(color);
				g.drawCircle(tabInset+tabW/2,tabW/2+7+0.5,tabW+1);
				g.endFill();
				g.beginFill(color);
				g.drawCircle(tabInset+tabW/2-2+(w - tabInset - tabW-1),tabW/2+7+0.5,tabW+1);
			}
			else g.drawCircle(tabInset+tabW/2,tabW/2+7,tabW);
			g.endFill();
		}
