...
//		}
	...
	//added
	public function countSpritesAndClones():int {
		// Return all sprites and clones with the given name.
		var result:Array = [];
		for (var i:int = 0; i < numChildren; i++) {
			var c:* = getChildAt(i);
			if (c is ScratchSprite) result.push(c);
		}
		var app:Scratch = parent as Scratch;
		if (app != null) {
			var spr:ScratchSprite = app.gh.carriedObj as ScratchSprite;
			if (spr) result.push(spr);
		}
		return result.length;
	}
	public function countSpritesAndClonesNamed(nm:String):int {
		// Return all sprites and clones with the given name.
		var result:Array = [];
		for (var i:int = 0; i < numChildren; i++) {
			var c:* = getChildAt(i);
			if ((c is ScratchSprite) && (c.objName == nm)) result.push(c);
		}
		var app:Scratch = parent as Scratch;
		if (app != null) {
			var spr:ScratchSprite = app.gh.carriedObj as ScratchSprite;
	...
	public function resetCloneIDs():void {
		// Return an array of all sprites in this project.
		var clones:Array = [];
		for (var i:int = 0; i < numChildren; i++) {
			var o:* = getChildAt(i);
			if (o is ScratchSprite) 
				if(!o.isClone)o.cloneCount = 0;
		}
		
		deleteClones();
		
	}
	public function spriteOrCloneVisible(num:int):Boolean {
		for (var i:int = 0; i < numChildren; i++) {
			var c:* = getChildAt(i);
			if (c is ScratchSprite){
				if (c.cloneID == num) return c.visible;
			}
		}
		return false;
	}
	...
	removeChild(c);
	if (c.bubble && c.bubble.parent) c.bubble.parent.removeChild(c.bubble);
	}*/
