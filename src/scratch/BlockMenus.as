...
public class BlockMenus implements DragClient {
	...
	public static function BlockMenuHandler(evt:MouseEvent, block:Block, blockArg:BlockArg = null, menuName:String = null):void {
		...
		//added
		if (menuName == 'print') menuHandler.printMenu(evt);
		if (menuName == 'case') menuHandler.caseMenu(evt);
		if (menuName == 'char') menuHandler.characterMenu(evt);
	}
...
//			handler.scrollAlignMenu(evt);
	...
	private function newBroadcast():void {
		...
		d.addText('Enter your message name in the text box');
		d.addAcceptCancelButtons('OK');
		d.showOnStage(app.stage);
	}
	...
	private function printMenu(evt:MouseEvent):void {
		var m:Menu = new Menu(setBlockArg, 'alignment');
		m.addItem('left');
		m.addItem('center');
		m.addItem('right');
		showMenu(m);
	}
	private function caseMenu(evt:MouseEvent):void {
		var m:Menu = new Menu(setBlockArg, 'case');
		m.addItem('大写');
		m.addItem('小写');
		m.addItem('首字母大写');
		showMenu(m);
	}
	private function characterMenu(evt:MouseEvent):void {
		var m:Menu = new Menu(setBlockArg, 'case');
		/*		m.addItem('♠');
		m.addItem('♣');
		m.addItem('♥');
	...
	private function istureMenu(evt:MouseEvent):void {
		var m:Menu = new Menu(setBlockArg, 'case');
		m.addItem('true');
		m.addItem('false');
		showMenu(m);
