...
public class PaletteBuilder {
	...
	public function showBlocksForCategory(selectedCategory:int, scrollToOrigin:Boolean, shiftKey:Boolean = false):void {
		...
		if (selectedCategory == Specs.myBlocksCategory) return showFunctionPalette(shiftKey);
	...
	private function showFunctionPalette(shiftKey:Boolean):void {
		// show creation button, hat, and call blocks
		var catColor:int = Specs.procedureColor;
		addItem(new Button(Translator.map('Make a Function'), makeNewFunction, false, '/help/studio/tips/blocks/make-a-block/'));
		var definitions:Array = app.viewedObj().procedureDefinitions();
		if (definitions.length > 0) {
			nextY += 5;
			for each (var proc:Block in definitions) {
				var b:Block = new Block(proc.spec, 'r', Specs.procedureColor, Specs.CALL, proc.defaultArgValues);
				addItem(b);
			}
			nextY += 10;
			addBlocksForCategory(Specs.myBlocksCategory, Specs.procedureColor);
		}

		//addExtensionButtons();	//Add for extension: comment by alex cui
...
//		}
	...
	private function makeNewFunction():void {
